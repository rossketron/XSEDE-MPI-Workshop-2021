...
int main(int argc, char *argv[]) {
    ...
    if (npes != NPES) {
        if (my_PE_num == 0) {
            printf("Incorrect PEs. Must run with %d PEs\n", NPES);
        }    
        MPI_Finalize();
        exit(1);
    }
    ...
    // PE 0 asks for input
    if (my_PE_num == 0) {
        printf("Enter max iterations [100-4000]: ");
        fflush(stdout);
    ...
    while ( dt_global > MAX_TEMP_ERROR && iteration <= max_iterations ) {
        ...
        // Send bottom row if not last PE 
        if (my_PE_num != npes-1) {
            MPI_Send(&Temperature[ROWS][1], COLUMNS, MPI_DOUBLE, my_PE_num+1, DOWN, MPI_COMM_WORLD);    
        }

        // Receive the data sent from above data segment and fill the top buffer row
        if (my_PE_num != 0) {
            MPI_Recv(&Temperature_last[0][1], COLUMNS, MPI_DOUBLE, my_PE_num-1, DOWN, MPI_COMM_WORLD, &status);    
        }

        // Send top row if not PE 0
        if (my_PE_num != 0) {
            MPI_Send(&Temperature[1][1], COLUMNS, MPI_DOUBLE, my_PE_num-1, UP, MPI_COMM_WORLD);    
        }

        // Receive the data sent from below data segment and fill the top buffer row
        if (my_PE_num != npes-1) {
            MPI_Recv(&Temperature[ROWS+1][1], COLUMNS, MPI_DOUBLE, my_PE_num+1, UP, MPI_COMM_WORLD, &status);    
        }
        ...
        // find global dt and broadcast to all PEs
...
void initialize(int npes, int mype)
{
    ...
    // Set bottom boundary of last PE
    if (mype == npes-1) {
        for (j = 0; j <= COLUMNS + 1; j++) {
            Temperature_last[ROWS+1][j] = j * 100.0/COLUMNS;
        }    
    }

    // Set left and right boundaries of every row for all PEs
    for (i = 0; i <= ROWS+1; i++) {
